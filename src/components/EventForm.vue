<template>
    <div id="event-form" :class="{ active: active }" :style="{ top: `${top}px`, left: `${left}px` }">
        ...
        <p>{{ date }}</p>
        <div class="text">
            <input v-focus type="text" v-model="description" placeholder="Build a flying car" @keyup.enter="create" >
        </div>
        <div class="buttons">
...
<script>
    export default {
        ...
        computed: {
            date() {
                return this.$moment(this.$store.state.eventFormDate).format('dddd, MMM Do');
            },
            active() {
